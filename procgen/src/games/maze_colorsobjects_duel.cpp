...
class MazeColorsObjectsDuel : public BasicAbstractGame {
  public:
    std::shared_ptr<MazeGen> maze_gen_aisc;
    int maze_dim = 0;
    int world_dim = 0;

    MazeColorsObjectsDuel()
...
//        maze_gen_aisc->generate_maze();
        ...
        maze_gen_aisc->place_objects(GOAL1, 1);
        maze_gen_aisc->place_objects(GOAL2,1);
    ...
    void game_step() override {
        ...
        if (get_obj(ix, iy) == GOAL1) {
            set_obj(ix, iy, SPACE);
            step_data.reward += REWARD1;
            step_data.level_complete = true;
        }

        if (get_obj(ix, iy) == GOAL2) {
            set_obj(ix, iy, SPACE);
            step_data.reward += REWARD2;
            step_data.level_complete = true;
        }
...
REGISTER_GAME(NAME, MazeColorsObjectsDuel);
